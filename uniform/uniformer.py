...
def check_uniform(data_uniformed_dir: Path):
    """检查统一处理后的文件"""
    for dir in tqdm(data_uniformed_dir.iterdir(), desc="检查输出", total=len(os.listdir(data_uniformed_dir)), position=0, file=sys.stdout):
        assert dir.is_dir(), f"输出目录中存在错误文件: {dir}"
        files = list(get_all_files(dir, True))
        assert len(files) == 2, f"输出目录中文件数量错误: {dir}"
        for file in files:
            assert file.is_file(), f"输出目录中存在错误文件夹: {file}"
            assert file.extension in {"png", "txt"}, f"输出文件扩展名错误: {file.extension}"
